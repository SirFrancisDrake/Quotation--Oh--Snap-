...
------------------------------------------------------------------------------
-- | Renders the quote page.
quote :: Handler App App ()
quote = do
    quotes <- liftIO $ wrapQuotes <$> fetchQuotes
    heistLocal (bindString "message" (T.pack quotes)) $ render "quotes"
    -- Blessed heist screens tags inside quotes. I don't honestly know why.
